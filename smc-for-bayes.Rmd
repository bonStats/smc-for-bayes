...
- *likelihood annealing* (a special case of geometric for static Bayesian models), where $\pi_{t}(\b\theta~\vert~\b{y}) \propto \pi(\b{y}~\vert~\b\theta)^{\phi_{t}}p(\b\theta)$, the likelihood function is $\pi(\b{y}~\vert~\b\theta)$, and $p(\b\theta)$ is the prior distribution. The annealing parameter $\phi_{t}$ is an in geometric annealing.
